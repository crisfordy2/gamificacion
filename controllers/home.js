    ...
    conn.query(consulta, (error, answer) => {
      ...
      conn.query(consultas, (error, result) => {
        ...
        const consulta2 = `INSERT INTO recordactivity (idActivity, idUser) VALUES ${usuarios}; SELECT * FROM activities WHERE idClass = ${data.idClass}; 
                SELECT u.idUser, u.nameUser, u.lastNameUser, a.idActivity , a.nameActivity,
                 r.idRecordActivity,COALESCE(r.noteActivity, 0) as noteActivity from users u 
                 INNER JOIN detailclass d ON u.idUser = d.idUser INNER JOIN class c ON c.idClass = d.idClass 
                 INNER JOIN activities a ON c.idClass = a.idClass INNER JOIN recordactivity r 
                 ON r.idActivity = a.idActivity WHERE c.idClass = ${data.idClass} and a.typeActivity = "Individual"
                and u.idUser = r.idUser order by a.nameActivity asc;`;
